...
class F1RaceReplayWindow(arcade.Window):
    ...
    def on_resize(self, width, height):
        ...
        for c in (self.leaderboard_comp, self.weather_comp, self.legend_comp, self.driver_info_comp, self.progress_bar_comp, self.race_controls_comp, self.prediction_panel):
            c.on_resize(self)
    ...
    def on_draw(self):
        ...
        # Prediction panel (toggle with 'P' key)
    ...
    def on_update(self, delta_time: float):
        ...
        if self._prediction_update_counter >= 25:
            ...
            self._cached_predictions = self.prediction_engine.calculate_all(
                self.frames, int(self.frame_index), max(0, laps_remaining)
            )
